...
function moverCavalo(pecaAnalisada, elementoDestino) {
    try {
        ...
        if (
            (Math.abs(parseInt(pecaAnalisada.linha) - parseInt(elementoDestino.dataset.line)) == 2 &&
            Math.abs(colunas.indexOf(pecaAnalisada.coluna) - colunas.indexOf(elementoDestino.dataset.column)) == 1) ||
            (Math.abs(parseInt(pecaAnalisada.linha) - parseInt(elementoDestino.dataset.line)) == 1 &&
            Math.abs(colunas.indexOf(pecaAnalisada.coluna) - colunas.indexOf(elementoDestino.dataset.column)) == 2)
        ) {
            permitirMovimento = true;
        }
        ...
        console.error("Erro ao mover o cavalo: ", e);
        return false;
    }
}
    ...
    try {
        ...
        if (pecaClicada.tagName == "IMG" || pecaClicada.tagName == "img") {
            if (pecaClicada.dataset.indexNumber) {
                // Adicione essa verificação para verificar se a peça clicada é um cavalo
                if (pecas[parseInt(pecaClicada.dataset.indexNumber)].tipo === "cavalo") {
                    // Verifique se o movimento é válido para o cavalo
                    if (moverCavalo(pecas[parseInt(pecaClicada.dataset.indexNumber)], pecaTemp.parentElement)) {
                        // Se o movimento for válido, faça o movimento
                        pecaTemp.appendChild(pecaClicada);
                    }
                }
            }
        }
    } catch (e) {
        console.error("Erro ao manipular clique do mouse: ", e);
    }
}
